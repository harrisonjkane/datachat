...
DATASETS = {
    ...
    "🏒 NHL Playoffs": {
        "file": "nhlplayoffs.csv",
        "questions": [
            "Which team has the most playoff wins?",
            "Show a bar chart of goals scored by team",
            "What is the average goals per game by year?",
            "Which year had the highest scoring playoffs?",
            "Show the win/loss record for each team",
        ],
    },
}

# Derive DEMO_CSV from the first dataset for backwards compatibility
DEMO_CSV = list(DATASETS.values())[0]["file"]
...
if "df" not in st.session_state:
    csv = DATASETS[st.session_state.selected_dataset]["file"]
    st.session_state.df = load_csv(csv)
if "df_name" not in st.session_state:
    st.session_state.df_name = DATASETS[st.session_state.selected_dataset]["file"]
def get_env_key():
...
with st.sidebar:
    ...
    if selected != st.session_state.selected_dataset:
        ...
        csv = DATASETS[selected]["file"]
        st.session_state.df = load_csv(csv)
        st.session_state.df_name = csv
        st.session_state.messages = []
        st.rerun()
    ...
    if st.session_state.df is not None:
        ...
        st.caption(f"{len(st.session_state.df):,} rows × {len(st.session_state.df.columns)} columns")
        st.download_button(
            label="⬇️ Download CSV",
            data=st.session_state.df.to_csv(index=False).encode("utf-8"),
            file_name=st.session_state.df_name,
            mime="text/csv",
    ...
    current_questions = DATASETS[st.session_state.selected_dataset]["questions"]
...
- For date parsing always use pd.to_datetime(df['col'], errors='coerce') — never assume date structure or use unit mappings like year/month/day separately. Do NOT use infer_datetime_format as it was removed in pandas 2.0
...
if st.session_state.df is None:
    ...
    using_shared_key = bool(get_env_key()) and not st.session_state.get("api_key_field", "")
